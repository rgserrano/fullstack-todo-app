...
app.post("/lists",
  ...
  requiresAuthentication, catchError(async (req, res) => {
...
app.get("/lists/:todoListId",
  requiresAuthentication, catchError(async (req, res, next) => {
    ...
    todoList.todos = await store.sortedTodos(+todoListId);
  ...
  requiresAuthentication, catchError(
    async (req, res, next) => {
      ...
      if (!errors.isEmpty()) {
        ...
        todoList.todos = await res.locals.store.sortedTodos(+todoListId);
